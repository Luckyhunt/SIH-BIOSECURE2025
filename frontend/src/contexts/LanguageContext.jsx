...
export const LanguageProvider = ({ children }) => {
    // Initial language check: Prefer localStorage, default to 'en'.
    // We REMOVE the complex hash checking to prevent the race condition/flicker.
    ...
    // Initialize state. We remove the 'isInitialized' state as it was for the flawed sync logic.
    const [language, setLanguage] = useState(getInitialLanguage)

    // The language selection from the custom selector updates localStorage and React state.
    const updateLanguage = (newLang) => {
        setLanguage(newLang);
        localStorage.setItem('selectedLanguage', newLang);
    };

    // We keep the hashchange listener for the case where the user uses the *default* Google Translate dropdown (if it were visible).
    // However, given your setup, this is mostly redundant as your custom selector handles the change.
    useEffect(() => {
      const handleHashChange = () => {
        const hashMatch = window.location.hash.match(/googtrans\(.{2}\|(\w{2})\)/) || 
                         window.location.hash.match(/googtrans\/\w{2}\/(\w{2})/)
        const hashLang = hashMatch ? (hashMatch[1] || hashMatch[2]) : 'en'

        if (hashLang && hashLang !== 'und' && hashLang !== 'en|en' && hashLang !== language) {
          // If Google changes the hash, sync React state
          setLanguage(hashLang) 
          localStorage.setItem('selectedLanguage', hashLang);
        } else if (hashLang === 'en|en' && language !== 'en') {
          // If Google reverts to English, sync React state
          setLanguage('en')
    ...
    // The core translation function: uses custom keys first, falls back to the key itself.
    const t = (key) => {
    ...
    const value = {
        language,
        setLanguage: updateLanguage, // Use the updated setter
        t,
        // isInitialized is removed
    }
